...
| [V] | 2026-03-26 | 5단계: LSP     |  9주차 | 13장     | 서브타이핑             | 직사각형/정사각형 분석                     | LSP 기준                   |
| [ ] | 2026-04-xx | 5단계: 패턴      | 10주차 | 14, 15장 | 일관성/패턴            | 조건·규칙 분리                         | 패턴의 본질                   |
|
...
| [V] | 2026-03-26 |  9주차 | 올바른 상속 기준        | LSP 위반 사례 설명 연습  |
| [ ] | 2026-04-xx | 10주차 | 일관성 있는 협력        | 패턴으로 정리          |
...
<details>
<summary> 5주차 (2026-02-25) </summary>
제공해주신 '9주 완성 플랜'과 책의 내용을 바탕으로 **5주차(6장, 8장)** 학습을 위한 **학습 목표, 실습 과제, 토론 주제**를 정리해 드립니다.
...
* **상황:** `Theater` 클래스가 `TicketSeller`의 내부 `TicketOffice`에 접근하고, 다시 `TicketOffice`가 `Ticket`에 접근하는 식의 코드를 찾거나 작성해 봅니다. (예: `ticketSeller.getTicketOffice().getTicket()`)
* **미션:**
    1. **디미터 법칙 위반 식별:** 점(.)이 연달아 나오는 코드를 찾아보세요.
    2. **'묻지 말고 시켜라' 적용:** `TicketSeller`에게 "티켓을 팔아라(`sellTo`)"라고 명령하도록 코드를 수정하여 내부 구현을 캡슐화합니다.
    3. **의도를 드러내는 인터페이스:** 메서드 이름을 `setTicket` 같은 데이터 중심의 이름에서 `buy`, `hold`, `sellTo`와 같이 협력의 의도가 드러나는 이름으로 변경합니다.

**과제 2: `new` 제거 및 의존성 주입 적용 (8장)**

* **상황:** `Movie` 클래스 내부에서 `AmountDiscountPolicy` 등을 `new`로 직접 생성하는 코드를 작성합니다.
* **미션:**
    1. **명시적 의존성으로 변환:** `new`를 제거하고, `DiscountPolicy` 타입을 **생성자**를 통해 주입받도록 수정합니다.
    2. **Setter 주입 실습:** 생성된 `Movie` 객체의 할인 정책을 런타임에 변경할 수 있도록 Setter 메서드(`changeDiscountPolicy`)를 추가해 봅니다.
    3. **컨텍스트 확장 확인:** 코드를 수정한 후, `Movie` 코드를 건드리지 않고 `PercentDiscountPolicy`나 `NoneDiscountPolicy`로 갈아끼우는 것이 얼마나 쉬워졌는지 확인합니다.
...
* **질문:** "책에서는 `ArrayList`나 `String` 같은 표준 라이브러리 클래스는 직접 `new`로 생성해도 문제가 없다고 말합니다. 구체 클래스에 의존하는 것이 위험한 이유는 '변경' 때문인데, 우리가 만드는 도메인 코드와 표준 라이브러리의 결정적인 차이는 무엇일까요?"

</details>

<details>
<summary> 6주차 (2026-03-05) </summary>
---
...
* **개방-폐쇄 원칙(OCP)의 메커니즘 이해:** OCP가 단순히 "확장엔 열려있고 수정엔 닫혀있다"는 구호가 아니라, **'컴파일타임 의존성'을 고정시키고 '런타임 의존성'을 변경**하는 기술적인 메커니즘임을 이해합니다.
* **생성과 사용의 분리(Separation of Use from Creation):** 객체가 협력할 대상을 직접 생성(`new`)하면 결합도가 높아짐을 깨닫고, 이를 해결하기 위해 **객체 생성 책임을 클라이언트나 FACTORY로 옮기는 법**을 배웁니다.
...
* **의존성 역전 원칙(DIP)과 패키지:** 상위 수준 모듈이 하위 수준 모듈에 의존하는 전통적인 구조를 뒤집어, 둘 다 **추상화**에 의존하게 만드는 원리를 배우고 이를 패키지 구조에 적용해 봅니다.

#### 2. 💻 실습 과제 (Hands-on Practice)

**과제 1: `new`의 해로움 체험 및 FACTORY 도입**

* **상황:** `Movie` 클래스 내부에서 `AmountDiscountPolicy`를 `new` 연산자로 직접 생성하는 코드를 작성합니다.
* **미션:**
    1. **문제점 확인:** 할인 정책을 변경하려면 `Movie` 코드를 수정해야 함을 확인합니다(OCP 위반).
    2. **책임 이관:** `Movie` 내부의 생성 로직을 제거하고, `Client`가 생성해서 `Movie` 생성자에 전달하도록 수정합니다.
    3. **FACTORY 패턴 적용:** 객체 생성 책임이 `Client`에 노출되는 것을 막기 위해, 생성만 전담하는 `Factory` 클래스(PURE FABRICATION)를 만들어 책임을 위임해 봅니다.

**과제 2: 숨겨진 의존성(Service Locator) 리팩터링**

* **상황:** 의존성 주입 대신, `ServiceLocator`라는 전역 객체를 통해 의존성을 해결하는 코드를 작성합니다.
* **미션:**
    1. **런타임 에러 유발:** `ServiceLocator`에 의존성을 등록하지 않은 상태로 코드를 실행하여 `NullPointerException`을 발생시켜 봅니다. 퍼블릭 인터페이스만 봐서는 무엇이 필요한지 알 수 없다는 문제(숨겨진 의존성)를 체감합니다.
    2. **명시적 의존성으로 전환:** 생성자 주입(Constructor Injection) 방식으로 변경하여, 필요한 의존성을 퍼블릭 인터페이스(생성자 파라미터)에 명확히 드러냅니다.

**과제 3: 의존성 역전(DIP)과 패키지 분리**

* **상황:** `Movie`가 구체적인 `AmountDiscountPolicy` 클래스에 의존하는 구조를 상상해 봅니다.
* **미션:**
    1. **인터페이스 도입:** `DiscountPolicy` 추상 클래스(또는 인터페이스)를 도입하여 `Movie`가 추상화에 의존하게 만듭니다.
    2. **패키지 구조 변경:** `Movie`와 `DiscountPolicy` 인터페이스를 같은 패키지(상위 수준)에 두고, 구현체인 `AmountDiscountPolicy`를 별도 패키지(하위 수준)로 분리하여 **SEPARATED INTERFACE 패턴**을 적용해 봅니다.
...
<details>
<summary> 7주차 (2026-03-12) </summary>
...
*   **상속의 치명적 단점 이해:** 상속이 단순히 코드를 복사하는 편한 도구가 아니라, 부모 클래스의 내부 구현에 자식 클래스를 강하게 결합시켜 **캡슐화를 위반**하게 만든다는 것을 배웁니다,.
...
*   **상황:** 10장에 나오는 일반 요금제(`Phone`)와 심야 할인 요금제(`NightlyDiscountPhone`)의 코드를 작성합니다,. 중복 코드를 없애기 위해 `NightlyDiscountPhone`이 `Phone`을 상속받도록 만듭니다.
*   **미션:** 
    1.  요구사항 변경: **통화 요금에 세금(`taxRate`)을 부과하는 기능**을 추가해 보세요.
    2.  결과 확인: 부모 클래스인 `Phone`에 세금 인스턴스 변수와 생성자를 추가했을 때, 자식 클래스인 `NightlyDiscountPhone`의 생성자까지 줄줄이 에러가 나며 수정해야 하는 상황을 눈으로 확인합니다,. 이를 통해 상속 계층 전체에 걸쳐 변경이 퍼져나가는 파급효과를 체감합니다.

**과제 2: 자바(Java) 표준 라이브러리의 흑역사 파헤치기**
*   **상황:** 불필요한 인터페이스 상속의 대표적 사례인 `java.util.Stack`과 `java.util.Properties`를 살펴봅니다.
*   **미션:**
    1.  IDE에서 `Stack<String>` 객체를 생성합니다.
    2.  스택의 기본 원칙인 LIFO(후입선출)를 무시하고, 부모 클래스인 `Vector`에서 상속받은 `add(index, element)` 메서드를 사용해 스택의 맨 밑이나 중간에 임의로 데이터를 밀어 넣어 보세요.
...
<details>
<summary> 8주차 (2026-03-19) </summary>
이번 주는 이전 주차(10장)에서 경험했던 상속의 치명적인 단점(클래스 폭발)을 **합성(Composition)**이라는 우아한 방법으로 해결하며, 왜 객체지향에서 "상속보다 합성을 선호하라"고 하는지 코드로 직접 증명하는 주간입니다.
---
...
*   **상황:** 10장에서 `java.util.Properties`나 `java.util.Stack`이 불필요한 인터페이스를 상속받아 문제가 생겼던 코드를 떠올려 봅니다.
*   **미션:** 
...
*   **미션:**
    ...
    2.  **합성 기반 설계:** `Phone` 클래스가 특정 구체 클래스를 상속받지 않고, `RatePolicy`를 생성자로 주입받아 내부 변수로 가지도록 수정합니다.
...
<details>
<summary> 9주차 (2026-03-26) </summary>
이번 주는 이전 주차에서 "상속을 피하라"고 배웠던 것을 넘어, **"그렇다면 상속은 도대체 언제, 어떻게 써야 하는가?"**에 대한 명확한 해답(타입 계층)을 내리는 매우 중요한 주차입니다. 그 핵심 기준이 바로 **리스코프 치환 원칙(LSP)**입니다.
...
*   **미션:**
    1.  자연어의 직관(`is-a`)에 따라 `Square`가 `Rectangle`을 상속(`extends`)받도록 작성하고, `Square`의 `setWidth`, `setHeight` 오버라이딩 시 너비와 높이가 항상 같도록 강제합니다.
    2.  **클라이언트의 가정 무너뜨리기:** 클라이언트 코드에서 `Rectangle`의 너비와 높이를 다르게 설정(`resize` 메서드)하도록 작성한 뒤, 그 파라미터에 `Square` 인스턴스를 넘겨봅니다.
...
<details>
<summary> 9주차 (2026-04-xx) </summary>
**5단계(10주차)**는 길었던 객체지향 설계 여행의 마지막 종착지입니다. 

이번 주차는 지금까지 배운 모든 원칙(다형성, 합성, 의존성 관리 등)을 종합하여 **'일관성 있는 협력'**을 만들고, 이러한 훌륭한 설계들의 모음인 **'디자인 패턴'과 '프레임워크'의 본질**을 이해하는 시간입니다.

제공해주신 커리큘럼과 14장, 15장의 핵심 내용을 바탕으로 학습 목표, 실습 과제, 토론 주제를 정리해 드립니다.

---

### 📅 10주차: 일관성 있는 협력과 디자인 패턴 (14, 15장)

**"디자인 패턴은 목적지가 아니라, 변경을 캡슐화하기 위한 나침반이다."**

#### 1. 🎯 학습 목표 (Learning Objectives)
*   **일관성 있는 협력의 중요성:** 유사한 기능을 서로 다른 방식으로 구현했을 때 발생하는 인지적 과부하를 이해하고, 협력을 일관성 있게 만들면 코드를 이해하고 확장하기 쉬워진다는 사실(개념적 무결성)을 배웁니다.
*   **조건과 규칙의 분리 (변경 캡슐화):** 변하는 개념을 변하지 않는 개념으로부터 분리하고, 변하는 개념을 캡슐화하는 설계 지침을 체득합니다.
*   **디자인 패턴의 본질 이해:** 디자인 패턴이 단순히 외워야 할 클래스 다이어그램이 아니라, 특정한 변경을 캡슐화하기 위해 반복적으로 재사용할 수 있는 **'역할과 책임, 협력의 템플릿'**임을 이해합니다.
*   **프레임워크와 제어 역전(IoC):** 프레임워크가 설계와 코드를 함께 재사용하는 방법이며, 상위 정책이 하위 세부 사항을 호출하는 '제어 역전(할리우드 원리)'을 통해 동작함을 이해합니다.

#### 2. 💻 실습 과제 (Hands-on Practice)

**과제 1: 핸드폰 과금 시스템의 '일관성' 리팩터링 (14장)**
*   **상황:** 시간대별, 요일별, 구간별 요금 방식을 각각 제각각의 논리(List, 개별 객체 등)로 구현하여 코드의 일관성이 무너진 상황입니다.
*   **미션:** 
    1.  변하지 않는 부분인 **'규칙(FeeRule)'**과 변하는 부분인 **'적용 조건(FeeCondition)'**을 분리합니다.
    2.  `FeeCondition`을 인터페이스로 추상화하여 변하는 부분을 캡슐화(서브타입 캡슐화, 객체 캡슐화)합니다.
    3.  `TimeOfDayFeeCondition`, `DayOfWeekFeeCondition` 등을 구현하여, 오직 변하는 부분만 추가해도 새로운 요금제가 기존의 일관된 협력 패턴에 자연스럽게 녹아드는 것을 확인합니다.

**과제 2: 내가 짠 코드에서 '디자인 패턴' 발견하기 (15장)**
*   **상황:** 2장부터 11장까지 만들었던 영화 예매 시스템과 핸드폰 과금 시스템 코드를 다시 펼쳐봅니다.
*   **미션:**
    1.  `Movie`와 `DiscountPolicy` 사이의 관계가 어떻게 알고리즘의 변경을 캡슐화하는 **STRATEGY (전략) 패턴**이 되는지 확인합니다.
    2.  합성 대신 상속을 사용하여 추상 메서드로 변경을 캡슐화한 부분이 **TEMPLATE METHOD 패턴**과 어떻게 일치하는지 분석합니다.
    3.  이를 통해 "패턴을 먼저 정하고 코드를 짠 것"이 아니라, "변경을 캡슐화하다 보니 패턴이 나타난 것"임을 깨닫습니다.

#### 3. 🗣️ 토론 주제 (Discussion Topics)

**주제 1: 억지스러운 일관성 vs 구조적 타협 (FixedFeeCondition)**
*   **질문:** "14장에서 '고정 요금제'는 사실 조건이 필요 없지만, 전체적인 협력 패턴(일관성)에 맞추기 위해 억지로 `FixedFeeCondition`이라는 조건 객체를 만들고 단 하나의 인스턴스만 반환하게 했습니다. 개념적 무결성을 지키기 위해 약간의 부조화(불필요한 객체 생성)를 수용하는 이런 설계 방식에 대해 어떻게 생각하시나요?"

**주제 2: '패턴 만능주의'의 함정**
*   **질문:** "초보 개발자들은 디자인 패턴을 배우고 나면 모든 문제에 패턴을 적용하려는 '망치를 들면 모든 것이 못으로 보이는' 패턴 만능주의에 빠지기 쉽습니다. 정당한 이유 없이 사용된 패턴은 설계를 복잡하게 만드는 장애물일 뿐입니다. 실무에서 패턴을 도입할 때, '단순함'과 '유연성' 사이의 트레이드오프를 어떻게 결정해야 할까요?"

**주제 3: 디자인 패턴과 프레임워크의 차이**
*   **질문:** "면접관이 '디자인 패턴과 프레임워크의 차이점이 무엇인가요?'라고 물어보았습니다. 15장에서 배운 내용(설계의 재사용 vs 설계와 코드의 재사용, 그리고 제어 역전 원리)을 바탕으로 명확하게 답변해 보세요."
</details>
